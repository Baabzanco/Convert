...
import { BMP_LIMITS, validateBmpFile } from '@/engines/shared/validation';
...
const STAGE_LABELS: Record<WorkerProgressStage, string> = {
  validating: 'Validating BMP...',
  reading: 'Reading file data...',
  decoding: 'Decoding bitmap...',
  encoding: 'Encoding PNG...',
  finalizing: 'Finalizing...',
};

export function BmpToPngController() {
  const [isHydrated, setIsHydrated] = useState(false);
  const [files, setFiles] = useState<BmpFileItem[]>([]);
  ...
  const handleFilesAdded = useCallback(
    async (incomingFiles: File[]) => {
      ...
      if (currentCount + incomingFiles.length > BMP_LIMITS.MAX_BATCH_FILES) {
        setGlobalMessage({
          type: 'error',
          text: `You can convert up to ${BMP_LIMITS.MAX_BATCH_FILES} files at a time. Only the first ${
            BMP_LIMITS.MAX_BATCH_FILES - currentCount
      ...
      const allowedIncoming = incomingFiles.slice(
        0,
        Math.max(0, BMP_LIMITS.MAX_BATCH_FILES - currentCount)
      );

      const validatedItems: BmpFileItem[] = [];
      ...
      for (const file of allowedIncoming) {
        ...
        if (ext !== 'bmp') {
          validatedItems.push({
            ...
            status: 'failed',
            progress: 0,
            errorMessage: 'Only BMP image files (.bmp) are supported.',
          });
          continue;
        }
        ...
        const val = await validateBmpFile(file);
        if (!val.valid) {
          validatedItems.push({
            id,
            file,
            status: 'failed',
            progress: 0,
            errorMessage: val.error?.message || 'This file is not a valid BMP image.',
          });
          continue;
        }
        ...
        validatedItems.push({
          ...
          status: 'pending',
          progress: 0,
          isTopDown: val.dimensions?.isTopDown,
  ...
  const convertBatch = async () => {
    ...
    for (const item of pendingFiles) {
      ...
      try {
        const result = await convertBmpToPng(item.file, (progress, stage) => {
          setFiles((prev) =>
        ...
        let msg = "We couldn't convert this BMP. Please try again.";
  ...
  const downloadSingle = (item: BmpFileItem) => {
  ...
  const downloadAllZip = async () => {
    ...
    try {
      ...
      triggerBlobDownload(zipBlob, 'bmp-to-png-files.zip');
    } catch {
      setGlobalMessage({
        type: 'error',
  ...
  return (
    <div
      id="bmp-to-png-tool-root"
      data-testid="bmp-to-png-tool-container"
    ...
    >
      ...
      {/* BMP notice callout */}
      <div
        id="bmp-format-notice"
        className="flex items-center gap-3 p-3.5 bg-sky-500/10 border border-sky-500/20 rounded-xl text-sky-900 dark:text-sky-200"
      >
        <Info className="w-4 h-4 shrink-0 text-sky-600 dark:text-sky-400" />
        <p className="text-xs sm:text-sm font-medium">
          <strong>Note:</strong> Standard 24-bit and 32-bit Windows Bitmaps (BMP) are converted directly inside your browser. Original pixel dimensions and aspect ratios are preserved.
        </p>
      </div>
      ...
      {files.length === 0 && (
        <div
          id="bmp-upload-dropzone"
          data-testid="bmp-dropzone"
          onDrop={onDrop}
        ...
        >
          <input
            ...
            id="bmp-file-input"
            type="file"
            accept=".bmp,image/bmp,image/x-bmp"
            multiple
            className="hidden"
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 text-xs text-zinc-400 dark:text-zinc-500">
            <span className="px-2.5 py-1 rounded-full bg-zinc-100 dark:bg-zinc-800 font-medium">BMP only</span>
            <span>•</span>
      ...
      {files.length > 0 && (
        <div id="bmp-workspace-section" className="space-y-6">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 p-4 bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
            ...
            <div className="flex items-center gap-2.5">
              ...
              {files.length < BMP_LIMITS.MAX_BATCH_FILES && (
                <>
                  <input
                    ref={addMoreInputRef}
                    id="bmp-add-more-input"
                    type="file"
                    accept=".bmp,image/bmp,image/x-bmp"
                    multiple
                    className="hidden"
          ...
          {/* Files List */}
          <div id="bmp-file-list" className="space-y-3">
            {files.map((item, index) => (
              <div
                key={item.id}
                id={`bmp-item-${index}`}
                data-testid={`bmp-file-item-${index}`}
                  ...
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex items-center gap-2 text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">
                        <span>BMP: {formatBytes(item.file.size)}</span>
            ...
            <div className="text-center sm:text-left">
              <h4 className="text-sm font-semibold">
                {completedCount === files.length && files.length > 0
                  ...
                  : `${files.length} BMP ${files.length === 1 ? 'file' : 'files'} ready to convert`}
              </h4>
              <p className="text-xs text-zinc-400 mt-0.5">
                {completedCount > 0
                  ? `${completedCount} of ${files.length} processed`
                  : 'Lossless PNG output with preserved pixel dimensions'}
              </p>
            </div>
