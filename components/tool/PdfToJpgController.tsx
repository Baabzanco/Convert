...
export function PdfToJpgController() {
  ...
  // Settings
  const [scale, setScale] = useState<PdfToJpgScale>(1.5);
  const [quality, setQuality] = useState<PdfToJpgQuality>('high');
  ...
  const handleFileSelected = useCallback(
    async (incomingFile: File) => {
      ...
      const validation = await validatePdfToJpgFile(incomingFile);
      if (!validation.valid) {
      ...
      try {
        ...
        if (loaded.pageCount > PDF_TO_JPG_LIMITS.MAX_PAGE_COUNT) {
          setGlobalMessage({
            type: 'info',
  ...
  const handleConvert = async () => {
    ...
    if (selectedPagesList.length === 0) {
      setGlobalMessage({
        type: 'error',
        text: 'Select at least one page to convert.',
      });
      return;
    }
    ...
    try {
      const convResult = await convertPdfToJpg(
        file,
        {
      ...
      setGlobalMessage({
        ...
        text:
          errAny?.message ||
          "We couldn't convert this PDF to JPG. Please try again with another file.",
      });
    } finally {
  ...
  const handleDownloadSingleJpg = (pageResult: { fileName: string; blob: Blob }) => {
  ...
  if (!isHydrated) {
    return (
      <div className="w-full max-w-4xl mx-auto p-8 text-center text-[#667085]">
        ...
        <p>Loading PDF to JPG Converter...</p>
      </div>
    );
  }
      ...
      <div data-hydrated={isHydrated ? 'true' : 'false'} className="w-full max-w-4xl mx-auto space-y-6">
        <div className="bg-white border border-[#E5E7EB] rounded-2xl p-6 sm:p-8 text-center shadow-sm">
          ...
          <p className="text-sm text-[#667085] mb-6 max-w-lg mx-auto">
            ...
            extracted as high-quality JPG images (Total: {formatBytes(result.totalConvertedSize)}).
          </p>
          ...
          <div className="text-left border-t border-[#E5E7EB] pt-6">
            <h3 className="text-sm font-bold text-[#17202A] uppercase tracking-wider mb-4">
              Individual JPG Files ({result.pages.length})
            </h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {result.pages.map((p) => (
                ...
                >
                  ...
                  <button
                    onClick={() => handleDownloadSingleJpg(p)}
      ...
      {!file ? (
        ...
        >
          ...
          <p className="text-sm text-[#667085] max-w-md mx-auto mb-4">
            Extract PDF document pages into high-resolution JPG images. Browser-based, fast, and 100% private.
          </p>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white border border-[#E5E7EB] rounded-2xl p-5 sm:p-6 shadow-sm space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              ...
              {/* Quality Settings */}
              <div>
                <label className="block text-xs font-bold text-[#17202A] mb-1.5">JPG Quality</label>
                <div className="grid grid-cols-3 gap-1 p-1 bg-[#F8FAFC] border border-[#E5E7EB] rounded-xl">
                  <button
                    type="button"
          ...
          <div className="bg-white border border-[#E5E7EB] rounded-2xl p-4 sm:p-6 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="text-xs text-[#667085] text-center sm:text-left">
              ...
              <span className="font-semibold text-[#17202A]">
                {selectedPagesList.length} {selectedPagesList.length === 1 ? 'JPG image' : 'JPG images'}
              </span>{' '}
              • {scale}× scale • {quality.toUpperCase()} quality
            </div>
              ...
              {isConverting ? (
                ...
                <>
                  ...
                  <span>
                    ...
                    {selectedPagesList.length === 1 ? 'Page to JPG' : 'Pages to JPG'}
                  </span>
                </>
              )}
            </button>
...
export default PdfToJpgController;
