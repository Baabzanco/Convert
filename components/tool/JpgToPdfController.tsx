...
import { JPG_TO_PDF_LIMITS, validateJpgToPdfFile } from '@/engines/pdf/validation';
import ProgressBar from './ProgressBar';

export interface JpgToPdfItem {
  id: string;
  file: File;
  previewUrl: string;
  width: number;
...
export function JpgToPdfController() {
  const [isHydrated, setIsHydrated] = useState(false);
  const [items, setItems] = useState<JpgToPdfItem[]>([]);
  ...
  const handleFilesAdded = useCallback(
    async (incomingFiles: File[]) => {
      ...
      if (currentCount + incomingFiles.length > JPG_TO_PDF_LIMITS.MAX_BATCH_FILES) {
        setGlobalMessage({
          type: 'error',
          text: `You can convert up to ${JPG_TO_PDF_LIMITS.MAX_BATCH_FILES} images at a time.`,
        });
      }

      const allowedIncoming = incomingFiles.slice(
        0,
        Math.max(0, JPG_TO_PDF_LIMITS.MAX_BATCH_FILES - currentCount)
      );

      const newItems: JpgToPdfItem[] = [];

      for (const file of allowedIncoming) {
        // Validate each file strictly as JPG / JPEG
        const validation = await validateJpgToPdfFile(file);
        if (!validation.valid) {
          setGlobalMessage({
            type: 'error',
            text:
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      const options: ImageToPdfOptions = {
        ...
        orientation,
        margin,
        outputFileName: filesToProcess.length === 1 ? undefined : 'jpg-to-pdf.pdf',
      };
  ...
  if (!isHydrated) {
    return (
      <div className="w-full max-w-4xl mx-auto p-8 text-center text-[#667085]">
        ...
        <p>Loading JPG to PDF Converter...</p>
      </div>
    );
  }
      ...
      <div data-hydrated={isHydrated ? 'true' : 'false'} className="w-full max-w-3xl mx-auto">
        <div className="bg-white border border-[#E5E7EB] rounded-2xl p-8 text-center shadow-sm">
          ...
          <p className="text-sm text-[#667085] mb-6">
            Your {result.pageCount} {result.pageCount === 1 ? 'JPG image has' : 'JPG images have'} been
    ...
    <div data-hydrated={isHydrated ? 'true' : 'false'} className="w-full max-w-4xl mx-auto space-y-6">
      ...
      {items.length === 0 ? (
        ...
        >
          ...
          <h2 className="text-xl font-bold text-[#17202A] mb-2">Select or Drop JPG Images to Convert</h2>
          <p className="text-sm text-[#667085] max-w-md mx-auto mb-4">
            Upload up to 20 JPG or JPEG photos to generate a clean, unified PDF. Browser-based
            & 100% private.
          </p>
          ...
          <p className="text-xs text-[#667085] mt-4">
            JPG and JPEG only • Max 50 MB per file • Up to 20 files per batch
          </p>
        </div>
      ) : (
          ...
          <div className="bg-white border border-[#E5E7EB] rounded-2xl p-4 sm:p-6 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              <h2 className="text-lg font-bold text-[#17202A]">
                {items.length} {items.length === 1 ? 'JPG Image Selected' : 'JPG Images Selected'}
              </h2>
            ...
            <div className="flex items-center space-x-3 w-full sm:w-auto">
              {items.length < JPG_TO_PDF_LIMITS.MAX_BATCH_FILES && (
                <button
...
export default JpgToPdfController;
