...
test.describe('Tool #19: PDF to PNG E2E', () => {
  ...
  test('Test 1: Single page — load page, upload one-page PDF, convert, verify 1 PNG result & download UI', async ({
    page,
  }) => {
    await page.goto('/tools/pdf-to-png');
    await page.locator('[data-hydrated="true"]').waitFor({ timeout: 10000 });
    await expect(page).toHaveTitle(/PDF to PNG Converter/i);
    await expect(page.locator('h1')).toHaveText(/Convert PDF to PNG/i);

    // Upload 1-page PDF
    const fileInput = page.locator('#pdf-to-png-file-input');
    ...
    // Click Convert
    const convertBtn = page.getByRole('button', { name: /Convert 1 Page to PNG/i });
    ...
    await expect(page.getByText('sample-1page-page-1.png')).toBeVisible();
  });

  test('Test 2: Multiple pages — upload 3-page PDF, select all, convert, verify 3 PNG results', async ({
    page,
  }) => {
    await page.goto('/tools/pdf-to-png');
    await page.locator('[data-hydrated="true"]').waitFor({ timeout: 10000 });
    ...
    // Click Convert
    const convertBtn = page.getByRole('button', { name: /Convert 3 Pages to PNG/i });
    await expect(convertBtn).toBeEnabled();
    await convertBtn.click();

    // Verify results
    await expect(page.getByText('PDF Converted Successfully')).toBeVisible({ timeout: 15000 });
    await expect(page.getByText('sample-3pages-page-1.png')).toBeVisible();
    await expect(page.getByText('sample-3pages-page-2.png')).toBeVisible();
    await expect(page.getByText('sample-3pages-page-3.png')).toBeVisible();
  ...
  test('Test 3: Page selection — upload 3-page PDF, select only Page 2, convert, verify single page result', async ({
    ...
    await expect(page.getByText('PDF Converted Successfully')).toBeVisible({ timeout: 15000 });
    await expect(page.getByText('sample-3pages-page-2.png')).toBeVisible();
    await expect(page.getByText('sample-3pages-page-1.png')).not.toBeVisible();
    await expect(page.getByText('sample-3pages-page-3.png')).not.toBeVisible();
  });

  test('Test 4: All pages — upload 3-page PDF, apply range 1-3, convert, verify all 3 PNG results', async ({
    page,
  }) => {
    await page.goto('/tools/pdf-to-png');
    await page.locator('[data-hydrated="true"]').waitFor({ timeout: 10000 });
    ...
    // Apply range 1-3
    const rangeInput = page.locator('input[placeholder*="1-3"]');
    await rangeInput.fill('1-3');
    const applyRangeBtn = page.getByRole('button', { name: /Apply Range/i });
  ...
  test('Test 5: Invalid file — upload PNG, verify rejection and friendly error', async ({
    page,
  }) => {
    await page.goto('/tools/pdf-to-png');
    await page.locator('[data-hydrated="true"]').waitFor({ timeout: 10000 });
  ...
  test('Test 7: Mobile viewport — render and convert at mobile viewport', async ({
    page,
  }) => {
